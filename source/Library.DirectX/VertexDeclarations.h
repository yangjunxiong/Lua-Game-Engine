...
namespace GameEngine
{
	...
	public:
		...
		static void CreateVertexBuffer(gsl::not_null<ID3D11Device*> device, const GameEngine::Mesh& mesh, gsl::not_null<ID3D11Buffer**> vertexBuffer);
	...
	void VertexDeclaration<T>::CreateVertexBuffer(gsl::not_null<ID3D11Device*> device, const gsl::span<const T>& vertices, gsl::not_null<ID3D11Buffer**> vertexBuffer)
	{
		...
		GameEngine::ThrowIfFailed(device->CreateBuffer(&vertexBufferDesc, &vertexSubResourceData, vertexBuffer), "ID3D11Device::CreateBuffer() failed.");
	}
}